...
try {
  ...
  await writeFile(
    ...
    [
      ...
      '',
      '// @ts-expect-error raw clients cannot promise JSON results',
      'const invalidJsonClient: HttpClient = rawClient',
      'void invalidJsonClient',
      'const dynamicClient: HttpClient<ResponseType> = rawClient',
      '// @ts-expect-error a dynamic client cannot promise JSON results',
      'const invalidDynamicClient: HttpClient = dynamicClient',
      'void invalidDynamicClient',
      '// @ts-expect-error extracted methods cannot narrow a dynamic default',
      "const invalidDynamicGet: HttpClient['get'] = dynamicClient.get",
      'void invalidDynamicGet',
      '',
      'function forwardOptions(options: RequestOptions) {',
      '  const direct = request<{ ok: boolean }>(\'https://api.example.com\', options)',
      '  const shared = client.request<{ ok: boolean }>(\'/users\', options)',
      '  // @ts-expect-error forwarded options may select a non-JSON mode',
      '  const invalid: Promise<{ ok: boolean } | undefined> = shared',
      '  void direct',
      '  void invalid',
      '}',
      'void forwardOptions',
